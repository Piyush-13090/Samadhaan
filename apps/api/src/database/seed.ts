...
const PROBLEMS = [
  ...
  },
  {
    ...
    // problem 3, described differently and dropped ~60 m away. The detector
    // should rank this as a likely duplicate on text, geography and category.
    n: 7,
    reporter: 1,
    title: 'Deep pothole beside the Main Market crossing',
    description:
      'There is a wide, deep pothole right beside the Main Market crossing. Bikes keep swerving around it into oncoming traffic and someone will be hit.',
    category: 'POTHOLES',
    subcategory: 'Road surface failure',
    status: 'SUBMITTED',
    severity: 'HIGH',
  ...
  },
  {
    ...
    // The control case — it must NOT be flagged, and it is why geography is a
    // precondition in the scorer rather than one signal among five.
    n: 8,
    reporter: 7,
    title: 'Deep pothole beside the main market crossing',
    description:
      'A wide, deep pothole has opened beside the main market crossing. Two-wheelers swerve around it into oncoming traffic every few minutes.',
    category: 'POTHOLES',
    subcategory: 'Road surface failure',
    status: 'SUBMITTED',
    severity: 'HIGH',
...
async function main(): Promise<void> {
  ...
  try {
    ...
    //
    // Until Prompt 8 this block inserted a pair with hand-written similarity
    // scores. Now that detection is real, fabricated scores would be worse than
    // none: they are indistinguishable from computed ones in the UI and in the
    // database, and they would be the first thing anyone tuning thresholds
    // measured against.
    //
    // The seed problems are built to demonstrate the detector instead —
    // problems 1 and 6 (waterlogging, ~25 m apart), 3 and 7 (the same pothole,
    // ~60 m apart) and 8 (problem 7's words, 230 km away, which must not
    // match). Generate the real candidates with:
    //
    //   npm run probe:duplicates --workspace @samadhaan/api
    //
    // or simply file a report through the UI, which runs the check on submit.
